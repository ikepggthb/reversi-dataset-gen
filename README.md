...
1 件のデータを作るには、目標の盤面から**終局まで対局を進める**必要があります。これを人間が手作業で行うことは現実的ではありません。何万件ものデータが必要なうえ、弱い手で進めると正確な評価値が得られないため、強い AI エンジンが必要です。

そこで **Edax** や **Egaroucid** のような強い Othello AI エンジンを使います。これらは高精度な探索を行うことができ、終局まで対局を進めて信頼性の高い評価値を生成できます。
...
`ai_probability` を 0 より大きくすると、ランダムではなく AI の手をある割合で混ぜられます。ただし値を上げるほど局面の多様性が下がります。
